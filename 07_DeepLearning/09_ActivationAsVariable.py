...
H = tf.keras.layers.Dense(
    128, activation=tf.keras.activations.swish)(H)  # 노드 수를 128로 늘림
...
H = tf.keras.layers.Dense(
    64, activation=tf.keras.activations.swish)(H)  # 또 다른 레이어 추가
H = tf.keras.layers.BatchNormalization()(H)  # 배치 정규화 적용
# 은닉 레이어 1에 Dropout을 추가. 50%의 노드를 랜덤하게 끔 (오버피팅 방지)
H = tf.keras.layers.Dropout(0.7)(H)
H = tf.keras.layers.Dense(
    32, activation=tf.keras.activations.swish)(H)  # 또 다른 레이어 추가
Y = tf.keras.layers.Dense(
    10, activation=tf.keras.activations.softmax)(H)  # 출력 레이어
